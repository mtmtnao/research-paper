...
- **結果**: MNIST と Frey Face で実験。Encoder/decoder の隠れユニット数は MNIST 500、Frey Face 200。$M=100,\,L=1$。
  ...
  - **周辺尤度の比較 (Fig.3, `graph_marglik`)**: 100 hidden units, 3 latent dims で、wake-sleep / MCEM (HMC sampler) と比較。AEVB は wake-sleep より良い周辺尤度に達し、MCEM は小データセットでは競合するが MNIST 全体には適用困難（オンラインでないため）。
  - **2D 潜在多様体の可視化 (Fig.4, appendix A)**: MNIST/Frey Face とも、prior $\mathcal{N}(\bzero,\bI)$ の逆 CDF で並べた格子を decoder に通すと、滑らかに変化する手書き数字/顔の多様体が得られる。
...
- **強み**:
  ...
  - 当時主流だった wake-sleep（2 目的関数）/ MCEM（per-datapoint sampling）に対して、**1 つの下界を 1 つの SGD で最適化**する設計はシンプル過ぎるほど。Fig.2 でほぼ全設定で wake-sleep を圧倒。
...
- **弱み / 疑問**:
  ...
  - 周辺尤度評価が 3 latent dims でしか信頼できない、と著者自ら認めている。Fig.3 の優位性は低次元レジームに限った主張で、Fig.2 の高次元（MNIST 200 dim）での「実際の test log-likelihood」は提示されていない（下界の比較のみ）。
