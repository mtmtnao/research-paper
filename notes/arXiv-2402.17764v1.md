...
- venue / year: TeX 中には明示なし（main.tex は neurips_2022.sty を preprint で使用）
...
- **学習時から ternary**: TeX では "trained from scratch, with 1.58-bit weights and 8-bit activations" と説明されており、post-training quantization として扱われていない。GPTQ/AWQ の延長線上にある手法ではなく、別系統として読む。
...
- **弱み / 疑問**:
  - **比較の透明性**: 比較対象は「著者ら再現の FP16 LLaMA を RedPajama 100B token で学習」。公開済みモデルとの直接比較ではなく、自前再現ベースラインとの比較である点に注意が要る。
  ...
  - **MMLU / GSM8K 等は評価に含まれていない**。評価タスクとして TeX に列挙されているのは ARC-Easy / ARC-Challenge / Hellaswag / Winogrande / PIQA / OpenbookQA / BoolQ、および 2T tokens 比較での Winogrande / PIQA / SciQ / LAMBADA / ARC-easy。
  - **学習コスト・学習安定性の議論が無い**。損失曲線・学習時間・収束性についての記述は TeX 中には見当たらない。
  ...
  - **専用ハードウェアは提案段階**。著者は New Hardware for 1-bit LLMs で専用ハード設計を呼びかけており、latency / memory は 2-bit kernel で測定されているため最適化余地が残る、と本文中で認めている。
  - **論文自体が短く limitations セクションが無い**。失敗例・崩壊条件・学習ハイパーパラメータ等の検証情報は TeX 中にはほぼ無い。
...
- **次に試したいこと**:
  ...
  - MMLU / GSM8K / HumanEval を実測し、reasoning / code タスクで FP16 との差が広がるかどうか確認（評者補足）。
  - "0" を入れた効果を切り分けるアブレーション: 同条件で binary {-1, +1}、ternary {-1, 0, +1}、quaternary {-1, -0.5, 0.5, 1} を比較し、本当に "feature filtering" が効いているかを sparsity 統計と性能で見る（評者補足）。
  - KV cache を 4-bit / 2-bit にしたときの長文タスク（needle in haystack, LongBench）での劣化を測る（評者補足）。
  - CPU / モバイル端末上で b1.58 3B を量子前後で動かして、論文の "edge/mobile" 主張を裏付けるベンチを取る（評者補足）。
...
- (verified 2026-05-27) venue/year を TeX で確認できる範囲（neurips_2022.sty preprint 使用、venue/year 明示なし）に限定 (main.tex)
- (verified 2026-05-27) LLaMA の学習 token 数、未評価 benchmark の性質、学習安定性の一般論など TeX 根拠外の記述を削除または評者補足として明示 (main.tex, main.bbl)
