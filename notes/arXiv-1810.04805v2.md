...
- **手法**: 多層双方向 Transformer encoder を、(1) **Masked LM (MLM)**：入力 WordPiece トークンの 15% をランダムに選び、そのうち 80% を `[MASK]` に置換・10% をランダム語に置換・10% は元のまま残して原語を当てさせる Cloze 風目的、(2) **Next Sentence Prediction (NSP)**：50/50 で A の次に来る本物 / ランダム文を `[CLS]` 表現から二値分類、の 2 タスクで同時事前学習する。fine-tuning では `[CLS]` ベクトル `C` または各トークン表現 `T_i` の上に最小限の output 層 1 枚を載せて end-to-end で全パラメータを再学習。事前学習・fine-tuning でアーキテクチャは出力層以外完全に共通。入力は token + segment + position の 3 種類の embedding の和、特殊トークン `[CLS]` / `[SEP]`、WordPiece 30,000 語彙、最大 512 トークン。モデルは `BERT_BASE` (L=12, H=768, A=12, 110M) と `BERT_LARGE` (L=24, H=1024, A=16, 340M)。事前学習データは BooksCorpus 800M words + English Wikipedia 2,500M words の合計 3.3B words、batch 256 系列 × 512 トークン = 128k tokens/batch を 1M step（約 40 epoch）、Adam lr=1e-4・β1=0.9・β2=0.999・weight decay 0.01・10k step warmup + linear decay、dropout 0.1、GELU。BERT_LARGE は Cloud TPU 64 チップで 4 日。
- **結果**: 11 NLP タスクで SOTA を一斉に更新。GLUE 平均 80.5（OpenAI GPT 72.8 から +7.7、Table 1：BERT_LARGE が 9 タスク全てで最高、BERT_BASE 79.6・BERT_LARGE 82.1 と本文の official 値で齟齬あり）。MNLI-m/mm 86.7/85.9、QNLI 92.7、SST-2 94.9、CoLA 60.5、STS-B 86.5、MRPC 89.3、RTE 70.1、QQP 72.1。SQuAD v1.1 Test F1 は単体 91.8 / アンサンブル+TriviaQA 93.2（Human 91.2、従来トップ nlnet ensemble 91.7、+1.5 F1）。SQuAD v2.0 Test F1 83.1（前 SOTA unet 74.9 から +5.1）。SWAG Test 86.3（ESIM+ELMo 59.2、OpenAI GPT 78.0、人間 88.0、著者 baseline 比 +27.1）。CoNLL-2003 NER Test F1 92.8（feature-based では top 4 hidden の concat で 96.1 Dev、fine-tune 比 -0.3 F1）。アブレーション (Table 5) では NSP を外すと QNLI 88.4→84.9, MNLI 84.4→83.9, SQuAD F1 88.5→87.9 と劣化、さらに MLM を LTR に変えると MRPC 86.7→77.5, SQuAD F1 87.9→77.8 と大幅悪化（BiLSTM を乗せても SQuAD 84.9 までしか戻らず、GLUE は逆に悪化）。モデルサイズ ablation (Table 6) では L=3,H=768 から L=24,H=1024 まで全 4 タスクで単調改善し、訓練データ 3,600 件の MRPC でも例外なく伸びる。
...
- **強み**:
  ...
  - fine-tune がシングル GPU で 1 時間以下、SQuAD で 30 分というコストの低さを明記（bert.tex 脚注）→ 公開重みの実用ハードルを意識的に下げにいっており、その後の普及（HuggingFace 等）の前提を作った。
...
- **弱み / 疑問**:
  ...
  - 「11 タスクで SOTA」のうち SQuAD v1.1 の +1.5 F1 はアンサンブル + TriviaQA 追加データ込みの比較で、単体・追加データ無しなら top leaderboard との差はかなり縮む。abstract の見出し数字はやや盛り気味。
  - GLUE 公式 Table の BERT_BASE 平均 79.6 vs 本文中 4.5% 改善という記述・アブストの「7.7% absolute improvement」など、複数の差分定義が混在していて読みにくい。
  ...
  - 計算資源（TPU v3 64 チップ × 4 日）が当時の academic 再現を事実上不可能にしており、その後「巨大事前学習 + 公開重み」の研究形態を固定化した功罪両方を負う論文。
- **次に試したいこと**:
...
- SQuAD v1.1 Test (Table 2)：BERT_LARGE Ens+TriviaQA EM 87.4 / F1 93.2、Human 82.3/91.2、トップ ensemble nlnet 86.0/91.7。
