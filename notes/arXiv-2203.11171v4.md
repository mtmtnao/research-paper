...
- **手法**: **Self-consistency** という新しい decoding 戦略。CoT prompt をそのまま使い、(1) decoder からサンプリングで多様な reasoning path $(\mathbf{r}_i, \mathbf{a}_i)$ を $m$ 個生成、(2) reasoning path を周辺化し、最終回答 $\mathbf{a}_i$ について **majority vote** を取る。追加学習・追加 verifier・人手アノテーション全て不要。同一の事前学習モデルの上で動くため "self-ensemble"。Table `tab:aggregation` で 5 種類の aggregation 戦略を比較し、PaLM-540B では **majority vote と normalized weighted sum がほぼ同等**（GSM8K でそれぞれ 74.4 / 74.1）、**normalized weighted avg だけが崩壊**（22.1）と示し、本文では実装が単純な majority vote を採用。
...
- **貢献**: (1) 教師信号も追加モジュールも不要な "sample-and-marginalize" decoding を提案、(2) reasoning 系統の幅広いベンチマーク (AddSub, MultiArith, ASDiv, AQuA, SVAMP, GSM8K, CSQA, StrategyQA, ARC-e/c, Letter, Coinflip) で当時の SoTA を多数更新、(3) decoding 多様性が beam search よりも reasoning 精度に効くこと、aggregation 重み付けはほぼ majority vote と等価であることを実証、(4) 「最終回答の一致率 = consistency」を **モデルの不確実性指標** として使える（Figure `fig:error_rate`: GSM8K で consistency と accuracy が高相関）と示唆。
...
- **重み付き集約と単純多数決がほぼ等価**: Table `tab:aggregation` で normalized weighted sum 74.1 vs majority vote 74.4。著者は「LLM は各経路の尤度を区別できるほど calibration されていない」と注釈で書いており、これは self-consistency が高 calibration を要求しないという実務上の強み。
...
- **CoT が逆効果な NLP タスクでも CoT+SC は standard prompt を超える**（Table `tab:common_nlp`）。CoT を「常に rationale を要求する一貫した枠組み」として再評価する根拠になる。
- **consistency 自体が confidence 推定**: Figure `fig:error_rate` で agreement 率と accuracy が高相関。verifier を別途学習する代わりに「多数決の票差」をそのまま不確実性スコアにできる。
...
- **強み**:
  ...
  - 4 種類の言語モデル × 算術・commonsense・symbolic reasoning タスクで検証している。算術では、本文が UL2-20B で +3%〜6%、LaMDA-137B と GPT-3 で +9%〜23% の絶対精度改善と述べており、Table `tab:sota` でも同傾向が示されている。
  - beam search・sample-and-rank・prompt ensemble・model ensemble の全てとの比較を載せており、「単に多数決を取っただけ」批判への先回りが効いている。
  - aggregation 戦略を Table `tab:aggregation` で 5 種類並べて比較し、計算量最小の majority vote を選んだ判断の透明性。
- **弱み / 疑問**:
  - 推論コストが path 数 × runs 倍になる（著者も Conclusion で limitation として明記）。Main Results は 10 runs × 40 outputs で報告されており、現実的な予算下での精度/コスト pareto は十分には示されていない。実用では path 数を 5〜10 に落とすことが推奨されているが、その点の定量比較は Figure `fig:exp_results` などに留まる。
  ...
  - confidence 指標としての consistency は GSM8K の Figure `fig:error_rate` に示されているが、TeX 中では ECE / Brier などの calibration 指標との定量比較は示されていない。
  - Reproducibility Statement で LaMDA-137B と PaLM-540B は not publicly available と明記されている。Appendix に prompt は載っているが、これら 2 モデルを含む完全再現には制約がある。
  - 多数決が**系統的な誤答を増幅する可能性**は実用上の懸念として残る（評者補足）。本論では、誤答に高 consistency が出る例の頻度や類型の体系的分析は示されていない。
- **次に試したいこと**:
  - 同じ token 予算で CoT-SC と他の複数推論経路系手法を比較する pareto curve を引く（評者補足。後続手法名はこの TeX には出ていないため省略）。
  - consistency をスカラ confidence として使い、ECE / selective accuracy などの指標で評価する（評者補足）。
  - 誤答に高 consistency が出る問題をクラスタリングし、systematic error を特定する（評者補足）。
  - sampling 多様性のために、温度・top-k・nucleus 以外の介入と SC を組み合わせた場合の追加ゲインを調べる（評者補足）。
  - SC で得られた多数派回答+rationale を教師信号として fine-tuning し、single inference run の精度を上げられるか検証する（Conclusion の future work に基づく）。
...
- aggregation 比較 (PaLM-540B, Table `tab:aggregation`): greedy 56.5 / majority vote 74.4 / normalized weighted sum 74.1 / unnormalized weighted avg 56.3 / normalized weighted avg 22.1（正規化平均だけ崩壊する）
...
- 設定詳細: 40 samples × 10 runs 平均, 標準偏差 ≤0.5 のため Table `tab:sota` / `tab:commonsense` では省略。temperature: UL2/LaMDA T=0.5 k=40, PaLM T=0.7 k=40, GPT-3 T=0.7 no top-k。
...
- (verified 2026-05-20) Summary の aggregation 比較で「unnormalized weighted avg が最悪 (22.1)」と誤っていたのを「normalized weighted avg が最悪 (22.1)」に訂正 (main.tex Table `tab:aggregation`: Weighted avg (unnormalized)=56.3, Weighted avg (normalized)=22.1)。
- (verified 2026-05-20) Related Papers の評価ベンチマーク列挙から「Hendrycks+ MMLU」を削除し、実際に使用されている MAWPS / ANLI / e-SNLI を追加 (main.tex §3.1 のタスク一覧、Table `tab:common_nlp` の NLP タスク。MMLU はこの論文では使用されていない)。
- (verified 2026-05-27) TeX 中に無い後続研究名・現況情報・未実施比較を削除または評者補足として明示し、Figure/Table 参照を TeX ラベルに合わせた (main.tex §3.5, Conclusion and Discussion, Reproducibility Statement, main.bbl)。

## Related Papers

- Wei+ 2022, "Chain of thought prompting elicits reasoning in large language models" — そのまま土台にしている。SC は CoT の decoding を差し替える形。
...
- Kojima+ 2022, "Large Language Models are Zero-Shot Reasoners" (Zero-shot CoT) — Table `tab:robustness` で組合せ実験。
