...
- venue / year: TeX 中には掲載先・年の明示なし（独自 `deepseek.cls` を使用し、`\figurename` を `Supplementary Fig.`、`\tablename` を `Supplementary Table` に再定義）
...
- **問題**: LLM の推論能力強化は CoT prompting や SFT に依存しがちで、人手で書かれた推論トレースの質・量にスケーラビリティが縛られる。さらに人間の思考様式を模倣させる枠組みでは、人間由来の例に制約されない推論経路の探索が妨げられる可能性がある。
- **手法**:
...
- **R1-Zero は CNMO/GPQA で R1 を上回る場面がある**（CNMO 88.1 vs 78.8、GPQA 75.8 vs 71.5）。一方で R1 は IF-Eval / AlpacaEval2.0 / ArenaHard などで大きく上がる。reasoning 単能力だけ欲しい場合に R1-Zero 系も候補に残す、というのは評者補足。
- **Aha moment は "wait" 出現率の急増として観測可能**（step 8000 以降）。これを training-time の軽量モニタリング指標として使う案は評者補足。
- **GRPO の利点**: value model が要らない＝メモリ・計算負荷を減らせる、かつ長 CoT で final reward から partial value を学ぶ困難を避けられる。PPO も λ=1.0 にチューニングすれば近づくが、著者は追加のハイパーパラメータ探索コストを指摘している。
- **蒸留 800K の内訳が公開**（reasoning 600K：math/code/STEM/logic、non-reasoning 200K：writing/factual/role-play）。これを SFT するだけで Qwen2.5-32B が QwQ-32B-Preview と Qwen2.5-32B-Zero を上回る。
- **報酬設計の知見**:
...
- **強み**:
  - 671B / 37B-activated MoE と \$294K でフロンティア性能、という再現可能性の説得力。
...
- **弱み / 疑問**:
  ...
  - **clip ratio ε=10** という大きい値。論文は「low だと token が大量に truncate される」「higher value may cause instability」と説明する。reference policy を 400 step ごとに更新している運用を含め、他環境で再現するかは評者補足として不明。
  - **総コスト \$294K** が "RL 単体" のものなのか "DeepSeek-V3-Base の事前学習を除いた値" なのか文面上明示が弱い（後者と読むのが自然だが、純粋に RL の手軽さを示したいなら base 込みの total を併記して欲しい）。
  - **蒸留評価で RL を加えない**: 「効果を示すだけが目的」と書き、R1-Distill + RL の探索は broader research community に委ねている。
  - **多 turn 対話**: 800K SFT が single-turn 中心で、multi-turn dialogue data の拡張は future work と明記。長期セッションでの挙動は評者補足として未検証。
  - **HarmBench の弱点**: risk control なしの DeepSeek R1 は HarmBench 35.0 で、著者は歌詞生成など IP 系拒否漏れを原因例として挙げる。一方、risk control ありでは 89.3、hide cot では 96.3 なので、単に「R1 が HarmBench で低い」とは言えない。
...
- **次に試したいこと**:
  - （評者補足）800K 蒸留データを使って自分の手元の 7B〜32B モデルを SFT、AIME/LiveCodeBench でどこまで再現できるか確認。
  - （評者補足）"wait" 出現率を training-time のオンライン指標として記録し、reward と共に early stopping/シード選択に利用する。
  - （評者補足）cold-start を **量を変えた ablation**（数百〜数万）でやり直し、reasoning 喪失（R1-Zero→Dev1 で AIME 77.9→59.0 のドロップ）の起点を特定。
  - （評者補足）GRPO の `Reward_language` を多言語タスクで設計し直すと、limitations に挙がっている非中英の language mixing が解けるか。
  - （評者補足）PRM を「再ランカ専用」（RL の報酬には使わない）に押し戻した上で、R1 出力の Pass@64 → Pass@1 を縮める実験（majority voting 79.8→86.7 がそのまま向上余地）。
...
- (verified 2026-05-27) venue/year を TeX で確認できる範囲に限定し、Codeforces percentile の「人類」表現、HarmBench の risk-control 条件、評者補足の明示、Related Papers の citation title 展開を修正 (sn-article.tex, appendix.tex, tables/safety_eval.tex)

## Related Papers

- `main.bbl` が存在しないため、citation key から論文タイトルを展開しない。TeX 本文で確認できる関連領域は、DeepSeek-V3 / DeepSeekMath、CoT、self-consistency、PRM、MCTS、distillation、test-time compute scaling、Tinyzero / Oat-Zero / open-r1。
