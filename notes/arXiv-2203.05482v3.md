...
- **結果**:
  - **ViT-G/14 (JFT-3B 事前学習) を ImageNet で fine-tune**: greedy soup が **90.94% top-1**（58 個中 14 個を選択）。当時の SOTA CoAtNet-7 の 90.88% を超え、25% 少ない FLOPs。val 最良単体は 90.78%、oracle (test 毎に最良) でも 90.78%。5 つの distribution shifts 平均で 85.02 vs 84.68 (oracle)。後発実験で **BASIC-L** でも 90.98% を達成。
  - **CLIP ViT-B/32 を ImageNet で fine-tune** (72 モデル random search, Table 2): 単体最良 80.38 / 47.83 (ImageNet / 5shift 平均) → uniform soup 79.97 / 51.45、greedy soup **81.03 / 50.75**。logit ensemble は 81.19 / 50.77、greedy ensemble は 81.90 / 49.44。soup は ID で ensemble に近く、OOD では ensemble を上回りやすい。
  - **ALIGN EfficientNet-L2**: 12 モデルから greedy soup が単体最良比 +0.5pp。
  - **NLP (GLUE, BERT/T5)** Table 3: T5 は MRPC 91.8→92.4, RTE 78.3→79.1, CoLA 58.8→60.2, SST-2 94.6→94.7。BERT も MRPC ±0, RTE 61.0→61.7, CoLA ±0, SST-2 92.5→93.0。改善は画像ほどではないが多くで正方向。
  - **cross-dataset soup** (付録 D): CLIP zero-shot + 6 データセット fine-tuned を soup して **CIFAR-100 zero-shot で +6.4pp**。
...
- **貢献**: (1) 異なる hp で独立 fine-tune したモデル群を「重み平均」だけで使い切る簡潔な recipe（特に greedy soup）、(2) ImageNet 90.94% という当時 SOTA、(3) ID/OOD/転移/NLP/zero-shot 横断で有効性を実証、(4) soup と ensemble の差を flatness × confidence で説明する解析、(5) WiSE-FT と直交し組み合わせ可能であることの提示（付録 C）。
...
- soup が効く条件は intuitively **同じ basin**（同じ $\theta_0$、暴れすぎない lr）。Figure 4 (`angles.pdf`) は「異なる hp ほど解がより直交 ($\phi$ が大) になり、平均の利得が大きい」を実証。seed だけ変えても角度は伸びにくく、aug / lr / 最適化器を散らす方が soup には効く。
...
- **WiSE-FT との重畳**: soup と zero-shot 初期値を更に線形補間すると OOD がさらに改善（§C, `robust_ft_small.pdf`）。つまり「hp で多様性 → 平均 → 初期値方向に少し引き戻す」の二段構え。
- **限界 (著者自身が §5 で明示)**:
  - 大規模・多様な事前学習が前提。ImageNet-22k 事前学習だと改善幅が大きく縮む（同じ hp sweep で `in22k.pdf`）。
  - **calibration は改善しない**。ensemble は ECE を下げるが soup は下げない（Figure C2 `varied_seed.pdf`）。confidence が必要な応用では ensemble の方が安全。
...
- **強み**:
  ...
  - ID と OOD の両方で改善するうえ、greedy soup は test を見ずに val だけで構成される。Table 1 で oracle (各 test set 別の単体最良) すら 5shift 平均 85.02 vs 84.68 で soup が上回る点は強い。
...
- **弱み / 疑問**:
  ...
  - 異なる random seed のみのモデルでは soup の伸びが小さい（Figure C2、Appendix `analysis0`）。多様性の源を意図的に設計する必要があるが、論文の hp sweep は基本的に既存実験の流用で、「soup を効かせるための sweep 設計」は将来課題のまま。
  ...
  - 理論近似は **線形に近い logit** の領域で導出されている。fine-tuning 後半で非線形性が強くなる場合（特に高 lr）に近似が外れる、というのは著者も Figure 7 で明示している。
- **次に試したいこと**:
  ...
  - cross-dataset soup（§D）の延長として、「downstream task 毎に専門 fine-tune した重みを動的に重みづけて soup」する task-conditional soup。
...
- ViT-G/14 結果: greedy soup 90.94 / Avg shifts 85.02, val 最良 90.72 / 84.38, oracle 90.78 / 84.68 (Table 1 = `tab:vit_g_finetuned_result`)
- CLIP ViT-B/32 (Table 2): best 80.38/47.83, uniform 79.97/51.45, greedy soup 81.03/50.75, ensemble 81.19/50.77, greedy ensemble 81.90/49.44
...
- ImageNet-22k 事前学習では gain が縮む: "the improvement is less substantial" (§5 Applicability, Appendix `in22k.pdf`)
- 角度 $\phi$ と interpolation advantage の相関 (Figure 4 = `angles.pdf`): hp を散らすほど $\phi$ が大きくなり soup の利得も上がる。
...
- Wortsman et al. 2021, WiSE-FT — fine-tune 後モデルと zero-shot 初期値の補間。soup と組み合わせ可能（§C）。
