...
- authors: Maxime Oquab, Timothée Darcet, Théo Moutakanni, Huy V. Vo, Marc Szafraniec, Vasil Khalidov, Pierre Fernandez, Daniel Haziza, Francisco Massa, Alaaeldin El-Nouby, Mahmoud Assran, Nicolas Ballas, Wojciech Galuba, Russell Howes, Po-Yao Huang, Shang-Wen Li, Ishan Misra, Michael Rabbat, Vasu Sharma, Gabriel Synnaeve, Hu Xu, Hervé Jegou, Julien Mairal, Patrick Labatut, Armand Joulin, Piotr Bojanowski（Julien Mairal は Inria affiliation、それ以外は Meta affiliation。Timothée Darcet / Pierre Fernandez は Inria co-affiliation、Théo Moutakanni は Université Paris Saclay co-affiliation、Alaaeldin El-Nouby は Inria / ENS-PSL co-affiliation）
...
- **手法**:
  ...
  - **データ (LVD-142M)**: 公開 web crawl から 1.2B unique images を集め、PCA hash で重複除去・NSFW フィルタ・顔ぼかし後、ImageNet-22k/1k・Google Landmarks・fine-grained / segmentation / depth / retrieval datasets を query として SSL ViT-H/16 (ImageNet-22k 事前学習) の embedding で cosine 近傍検索と k-means クラスタからの sampling を組み合わせ、142,109,386 images の curated set を構築。sample-based retrieval は Google Landmarks v2 / ImageNet-22k で k=4、ImageNet-1k train で k=32、cluster-based retrieval は小規模 dataset ごとに最大 1M images。Faiss + 20 ノード×8 V100 で 2 日以内。
...
- LVD-142M は uncurated より良いだけでなく ImageNet-22k より広い domain（iNat, Places）で良い。「curated データを retrieval で疑似的に拡大する」アイデアは、著者が CCNet 系の text curation pipeline からの着想として述べている image-similarity ベースの curation。
...
- **弱み / 疑問**:
  ...
  - **「curated データ」が ImageNet-22k 等の人手キュレーション済セットを query とする以上、ImageNet 系の class 分布が bottleneck になり得る**（評者補足）。retrieval された web 画像も query 近傍に偏るはずで、本当に "general" な分布なのかは TeX 中の議論だけだと不十分（iNat / Places で勝つ、という間接証拠に頼っている）。
  ...
  - **distillation で ViT-L が ViT-g とほぼ並ぶ**のは強みである一方、「結局 1B モデルが必要だったのか？」という疑問を惹起する。同じ計算で複数の中規模 SSL を ensemble する方が良かった可能性は TeX 中では議論されていない（評者補足）。
  - SSv2 では iBOT-L (38.7) が ViT-g (38.3) に僅差で勝つ。temporal motion を要する task では本手法が必ずしも勝たない＝静止画 SSL の限界として正直に出ている。
  - test set 漏洩対策として benchmark validation/test との near-duplicate を除去している記述はあるが、ImageNet-1k を query に curation している以上、評価セットとの "概念的" 漏洩は完全には排除できないはず（評者補足）。
- **次に試したいこと**:
  - DINOv2 features を入力 token として LLM に流す（著者自身が future work で言及）。VLM の vision tower を CLIP から差し替えるとどこで勝ち負けが反転するかは興味深い。
  - LVD-142M の query set を ImageNet 系から外し（例: iNaturalist のみ、Open Images のみ）、curation pipeline 自体が "ImageNet 様の分布" にどれだけ依存しているか測る（評者補足）。
  - KoLeo 正則化を CLIP の image-encoder にも入れたとき instance retrieval が改善するか（評者補足。テキスト埋め込みとの整合は崩れる可能性あり）。
  - distillation を多段（g→L→B→S）にして、各段で精度がどう劣化するかの curve を引く。1 段 distill が最適という仮定は ablate されていない（評者補足）。
  - "frozen + linear" を保ったまま、ADE20k で Mask2former + ViT-Adapter のように decoder 側だけ強化したときの pareto frontier。本論文の 60.2 mIoU と SoTA 62.9 mIoU の差が decoder 由来か backbone 由来かを isolate したい（評者補足）。
...
- LVD-142M: 1.2B unique images から、dedup / benchmark validation-test near-duplicate 除去 / retrieval を経て 142,109,386 images。sample-based retrieval は Google Landmarks v2 / ImageNet-22k で k=4、ImageNet-1k train で k=32。Faiss + 20 nodes × 8 V100, 2 日以内 (data.tex, supp-data.tex)。
...
- (verified 2026-05-27) LVD-142M の retrieval 個数を一律 N=4 から、Google Landmarks v2 / ImageNet-22k は k=4、ImageNet-1k train は k=32、cluster-based retrieval は最大 1M images に修正 (data.tex, supp-data.tex)。
- (verified 2026-05-27) TeX 中の明示事実でない Critical Thoughts / 次に試したいことの推論に「評者補足」を明記 (data.tex, supp-eval-datasets.tex, experiments.tex)。
- (verified 2026-05-27) authors の affiliation を tmlr.tex の author block / footnote に合わせて修正 (tmlr.tex)。

## Related Papers

- Caron+ 2021, *Emerging properties in self-supervised vision transformers* (DINO) — image-level loss の直接の元。
- Zhou+ 2022, *ibot: Image bert pre-training with online tokenizer* — patch-level masked prediction の base。本論文は iBOT を起点に scaling した位置付け。
- Caron+ 2020, *Unsupervised learning of visual features by contrasting cluster assignments* (SwAV) — Sinkhorn-Knopp 中心化の出典。
- He+ 2022, *Masked autoencoders are scalable vision learners* (MAE) — finetune 前提の SSL baseline。frozen 比較で大差負け。
- Radford+ 2021, *Learning transferable visual models from natural language supervision* / Ilharco+ 2021, *Openclip* / Fang+ 2023, *Eva: Exploring the limits of masked visual representation learning at scale* — text-guided weakly-supervised の主要比較対象。
- Goyal+ 2022, *Vision models are more robust and fair when pretrained on uncurated images without supervision* (SEERv2) — uncurated 大規模 SSL の代表 baseline。
- Sablayrolles+ 2019, *Spreading vectors for similarity search* — KoLeo 正則化の出典。
- Dao+ 2022, *Flashattention: Fast and memory-efficient exact attention with io-awareness* — 独自実装の元。
- Touvron+ 2019, *Fixing the train-test resolution discrepancy* / Beyer+ 2023, *Flexivit: One model for all patch sizes* — 末期高解像 adaptation の思想的源流。
- Hinton+ 2014, *Distilling the knowledge in a neural network* / Duval+ 2023, *A simple recipe for competitive low-compute self supervised vision models* — distillation 手法の元。
- Wenzek+ 2020, *Ccnet: Extracting high quality monolingual datasets from web crawl data* — テキスト側の curation pipeline、本論文の image retrieval ベース curation のアナロジー元。
