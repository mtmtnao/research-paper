...
- venue / year: TeX 中には明示なし（main_arxiv.tex は neurips_2023.sty を preprint で使用）
...
- **結果**: 7 タスク（Dialogue Response Generation, Code Optimization, Code Readability Improvement, GSM8K (Math Reasoning), Sentiment Reversal, Acronym Generation, CommonGen-Hard）× 3 ベース LLM（GPT-3.5 text-davinci-003, ChatGPT gpt-3.5-turbo, GPT-4）で評価。Table 1 では全 21 セル中 20 セルでベースを上回り、平均約 20% 絶対改善。代表例（GPT-4 base → +Self-Refine）: Dialogue Response 25.4→74.6 (+49.2)、Sentiment Reversal 3.8→36.2 (+32.4)、CommonGen-Hard 15.0→45.0 (+30.0)、Code Readability 27.4→56.2 (+28.8)、Acronym 30.4→56.0 (+25.6)、Code Optimization 27.3→36.0 (+8.7)、GSM8K 92.9→93.1 (+0.2)。GSM8K だけほぼ無改善（後述）。Introduction では Codex (code-davinci-002) のような code model で最大 13% absolute 改善とも述べている。
...
- **同じモデル**だけで生成・批評・修正を全部回せる、追加学習も外部 reward も不要というのが本質。
- **「actionable かつ specific」が feedback の核**。Table 3 のアブレーション（Sentiment Reversal: 43.2 vs generic 31.2 vs no-feedback 0、Code Opt: 27.5 vs 26.0 vs 24.8、Acronym: 56.4 vs 54.0 vs 48.0）が示すのは、「Improve the efficiency」のような generic な自己反省より、具体的なフィードバックの方が高スコアになること。Reflection 系手法を組むときの実装指針として強い（評者補足）。
- 反復による gain は **early iterations に集中**（Fig. 4: Constrained Gen は $y_0\to y_1$ で +11.3、$y_1\to y_2$ で +6.4、$y_2\to y_3$ で +3.0 と diminishing returns）。コスト最適化なら 1-2 round で打ち切るのが候補になる（評者補足）。
- **GSM8K の停滞 (92.9→93.1, +0.2)** は重要。ChatGPT は 94% の事例で feedback を "everything looks good" と返してしまう。つまり「自分のミスを自分で見つける」能力が弱いタスクでは self-refine は機能しにくい。外部検証器（compiler, calculator, unit test）と組む方向が考えられる（評者補足）。
- **1 vs $k$ sampling 比較**: $k=4$ 個独立にサンプルさせて best-of-k と比べても、Self-Refine がそれら全部より好まれる（§Analysis）。「多数生成して選ぶ」より「1 本を直す」方が有効な可能性を示す。
...
- **弱み / 疑問**:
  ...
  - **GSM8K でほぼ改善しない**ことを著者自身が認め、「外部信号があれば +5% 以上」と注釈している。事実検証や算術のように「正答可否を自己判断しにくいタスク」では、少なくともこの設定の純 self-refine は改善が小さい。
  - **多くの評価が GPT-4-as-judge**。Dialogue / Sentiment / Acronym は GPT-4-pref（人間との一致は Sentiment 82%、Acronym 68%、Dialogue 71%）を用い、Code Readability でも GPT-4 による変数名評価を使っている。生成器と判定器を同じ系列モデルで使っている circularity がある。著者は人間 A/B も併用しているがサブセットのみ。
...
- **次に試したいこと**:
  - 同 token 予算で self-consistency / Tree-of-Thoughts / Reflexion / multi-agent debate との pareto curve（精度 vs total tokens）を引く（TeX 中には明示されていない / 評者補足）。
  - GSM8K で feedback 役だけ外部検証器（Python 実行 / calculator）に置き換えるハイブリッド構成を組み、純 LLM-feedback と分離評価（Oracle Feedback 実験に基づく評者補足）。
  - feedback prompt の「actionable・specific」要件を満たしているかを LLM で自動判定させ、不合格時に feedback だけリトライする 2 段構成で改善するか（TeX 中には明示されていない / 評者補足）。
  - Self-Refine の対話ログを SFT データに distill して、小モデル単発生成で大モデル + Self-Refine の出力品質にどこまで迫れるか（TeX 中には明示されていない / 評者補足）。
  - 「stop score」の校正実験：iteration 早期に "no further refinement needed" と判断したケースの真の品質を測り、停止判定の precision/recall を出す（TeX 中には明示されていない / 評者補足）。
...
- (verified 2026-05-27) venue/year を TeX で確認できる範囲（main_arxiv.tex は neurips_2023.sty を preprint option で使用、掲載先明示なし）に限定し、GPT-4 評価の記述を Metrics の記述に合わせて修正（main_arxiv.tex, sections/500_results.tex）。
- (verified 2026-05-27) 評者独自の次実験案には「評者補足」を明記し、Related Papers のデータセット名を TeX/bbl で確認できる範囲に限定（main_arxiv.bbl, sections/appendix/task_details/sentiment.tex, sections/appendix/additional_analysis.tex）。
- (verified 2026-05-27) Takeaway の実装・運用上の解釈を TeX 事実と区別できるように削除または「評者補足」へ変更し、Codex 関連の結果記述を Introduction の "up to absolute 13%" に合わせた（sections/200_intro.tex, sections/600_analysis.tex）。
...
- Cobbe+ 2021 Math Reasoning dataset, Lin+ 2019 CommonGen, Mehri & Eskenazi 2020 FED (Dialogue), Zhang+ 2015 sentiment dataset — 評価データセット。
- PIE (Madaan+) — Code Optimization タスクの出典で baseline。
- Chiang+ 2023 Vicuna — 弱モデル比較対象。
- Brown+ 2020 "Language models are few-shot learners" — few-shot in-context learning の基礎。
