...
- **手法**: sub-billion 領域で重要なのは「データ量・パラメータ数」よりも「アーキテクチャ」だと主張し、次の4つの設計を組み合わせた **MobileLLM** を baseline として作る。(1) FFN を SwiGLU 化（125M で avg 42.6→43.9）、(2) **deep-and-thin**（layer 数 30〜32 を採用、scaling law の常識である 12 layer 系を否定）、(3) 入出力 **embedding sharing**（vocab 32k×embed 512 で 16M パラ削減、約11.8%）、(4) **grouped query attention**（head sweep 結果としては 16 query head / 4 kv-head が良いが、最終 MobileLLM-125M は L=30/H=9/KV=3/Emb=576、MobileLLM-350M は L=32/H=15/KV=5/Emb=960 で head_dim=64、appendix Table tab:larger_architecture）。さらに immediate **block-wise layer sharing**（隣接 2 ブロックで重みを共有、計算は 2 回行うが SRAM 上に置けるので DRAM↔SRAM の重み転送を回避）を追加した版を **MobileLLM-LS** と呼ぶ。学習は 32×A100、各 GPU bs=32、Adam wd=0.1、lr=2e-3 cosine、120k iter / 0.25T tok で探索 → 480k iter / **1T tokens** で最終モデル。学習データの種類は本文では明示されていない。
- **結果**: zero-shot common sense reasoning 平均（main results table tab:main）で **MobileLLM-125M 46.3 / -LS-125M 47.0**（先行 best RWKV-169M 43.6、Pythia-160M 42.5 を 2.7〜3.8pt 上回り、しかも 22〜26% 小さい）、**MobileLLM-350M 51.3 / -LS-350M 52.1**（先行 best RWKV-430M 47.0 を +4 以上）。layer sharing が +0.7 / +0.8。TQA F1 (tab:more_task) は -125M 1-shot 13.9（OPT-125M 8.7）、-350M 1-shot 22.0（OPT-350M 11.0、約 +10pt）。RACE middle/high も同様に大幅勝ち。Chat（tab:chat）: AlpacaEval 勝率 **MobileLLM-LS-350M 48.20%** が text-davinci-001（自己勝率 50%）に肉薄、Falcon-1.3B 30.38%・OPT-1.3B 38.84% を上回り 1B 超のモデルすら抜く。MT-Bench も -LS-125M 2.52、-LS-350M 3.16（OPT-125M 1.21 比）。API calling（tab:api）: **MobileLLM-350M の EM_intent 65.3 / EM_structure 48.8** は LLaMA-v2 7B（62.8 / 50.9）に同等以上（Rouge は 7B が上）。iPhone 13 ExecuTorch + MPS 実測（tab:latency）: MobileLLM-LS の load 43.6ms / init 1388.2ms / execute 16.0ms は MobileLLM 比 +2.2%（load+init 合算）/ +2.6%（execute）に対し、純粋に 60 層に倍化した non-shared モデルは load+init +143% / execute +86%（個別では load +75% / init +146% / execute +86%）。W8A8 PTQ も accuracy drop <0.5pt（§3.4）。Appendix では 600M / 1B / 1.5B にスケールしても MobileLLM-1.5B avg 59.4 が Qwen1.5-1.8B を 2.9pt 上回る。
...
- 最終アーキ構成（appendix tab:larger_architecture）: MobileLLM-125M = L30/H9/KV3/Emb576/Hidden1536/124.6M、MobileLLM-350M = L32/H15/KV5/Emb960/Hidden2560/345.3M。head_dim=64、embedding shared。head sweep (§2.2.4) では 16 query head / 4 kv-head が良いと述べる一方、最終モデルは異なる head 数を使う点に注意。
- tab:main: MobileLLM-LS-125M (30 distinct layers) avg **47.0** > 既存 350M 級モデル多数（Pythia-410M 46.6、BLOOM-560M 44.2）。
- tab:latency 実測: MobileLLM-LS は 60-layer 非共有版より load 36.5%、init 58.5%、execute 44.8% 速い（ExecuTorch + MPS / iPhone 13）。
- Layer sharing ablation（tab:layer_share）で immediate block-wise は repeat-all-over に僅差で負けるが、SRAM ローカリティ理由で immediate を選択。
- 学習: Adam wd=0.1、lr=2e-3 cosine、32×A100、bs=32/GPU、最終 480k iter × 1T tokens。
- Appendix §G: KD with LLaMA-v2 7B teacher → 2.6–3.2× slower (125M: 29h→93h, 350M: 42h→109h)、avg accuracy は 125M 43.9→43.8、350M 49.1→48.8 と同等以下 → 採用しない。
- §3.4 quantization: W8A8 PTQ で accuracy drop <0.5 pt、layer sharing と互換。
- (verified 2026-05-20) 最終アーキの head/kv-head を 16/4 → 9/3 (125M), 15/5 (350M) に訂正 (appendix tab:larger_architecture / appendix/01_large_size_architecture.tex)。16/4 は §2.2.4 の head-sweep 結果で別 baseline。
- (verified 2026-05-20) KD / Quantization のセクション番号を §4.x → §3.x に訂正 (main.tex の §3 Experiments 構造)。KD の appendix 参照を F → G に訂正 (appendix.tex 章順)。
...
- Dauphin+ 2017 (`dauphin2017language`) — 本論文が SwiGLU の根拠としている唯一の citation（Shazeer 2020 は bbl に未登録）。
