...
- **「ニューロンごとに私的な MLP $g_{\theta_d}$ を持たせる」**ことで、各 neuron が自分自身の過去 $M$ tick の pre-activation 履歴から post-activation を作る、という設計になっている。著者はこれを neuron-level temporal processing / NLM として CTM の中核要素に置いている。
...
- **Q&A MNIST で「観測した数字が memory window $M$ の外に出ても recall できる」**結果は、著者が synchronization 表現を memory / recall の機構として解釈している箇所。同期パターンを記憶に使う方向性は、著者の Future Work にある biologically-inspired memory とも接続する。
...
- **ImageNet / CIFAR-10 で calibration を分析している**点は、単なる accuracy 以外の CTM の挙動を見る材料として重要。ImageNet では tick ごとの calibration plot、CIFAR-10 では human baseline との比較が示されている。
...
- **強み**:
  ...
  - 1 つのアーキテクチャで maze / ImageNet / parity / CIFAR / Q&A / sort / RL を一通り回している breadth は誠実。parity / CIFAR / Q&A / maze ablation / RL では LSTM や FF との比較、または parameter / tick を近づけた比較が示されている。
...
- **弱み / 疑問**:
  ...
  - **Parity の seed 分散が大きい**: 75 tick / memory 25 の 3 seed のうち 1 つは suboptimal に収束（Fig. \ref{fig:appendix/parity/training}）。学習の安定性が seed に強く依存する点は弱い。
  - **比較対象が LSTM / FF にほぼ限定**。PonderNet / ACT / Looped Transformers / Sparse Universal Transformers / Quiet-STaR は Related Work で参照されるが、同条件実験は TeX 中には示されていない。adaptive compute と sequential reasoning の比較対象としては不足が残る。
  - **同期行列 $D\times D$ のサンプリング戦略がタスクごとに違う**（Dense / Semi-dense / Random）が、なぜその選択になるのかの原理的説明は TeX 中には明確でない（評者補足: tuning artifact かどうかは未検証）。
  - **NLM のパラメータ数増加**を著者は limitation として認めるが、同じ計算・パラメータ予算を別の強い recurrent / iterative baseline に振った場合との pareto は TeX 中には示されていない（評者補足）。
  - **2 tick 損失 ($t_1$, $t_2$)** が学習を駆動するが、$t_1=\arg\min\mathcal{L}$ は **真ラベルを使う argmin** なので推論時には存在しない。学習と推論の停止規準が一致しない設計の良し悪しは議論が薄い。
  - **「neural synchronization が記憶」**という主張は Q&A MNIST で示されているが、Q&A MNIST では synch なし baseline との比較は TeX 中には示されていないので、本当に synchronization 由来かは未確定（評者補足）。
...
- **次に試したいこと**:
  - （評者補足）同じ計算予算で **Looped Transformer + early-exit** などの iterative baseline と CTM を maze / parity / Q&A MNIST で並べた pareto。「neural dynamics 表現」ではなく「単に深く回したから解けた」可能性を切る。
  - （評者補足）**NLM を学習可能 activation function として既存 Transformer に差し込む** ablation。「neuron 私的 MLP の効果」だけを Transformer ハーネスで測れば、CTM 全体ではなく要素ごとの寄与が分かる。
  - **同期行列の指数減衰 $r_{ij}$ の学習結果を可視化**して、タスクごとに有効な時間スケールがどう分布するか（maze は使う / ImageNet はほぼ使わないという脚注の数値化）。
  - （評者補足）**adaptive compute の `over-thinking`**（ImageNet 21202 の例で正解 → 不正解と漂流する現象、Fig. \ref{fig:imagenet-21202}）を停止規準で抑える手法。
  - **言語モデリングへの応用**（著者 future work）。tick を「思考ステップ」として推論時 scaling に使えるかは、TeX 中には実験されていない今後の確認点。
...
- (verified 2026-05-27) Takeaway / Critical Thoughts から TeX 根拠のない ResNet-152 単体との calibration 比較を削除し、評者補足の提案・推論を明示 (main.tex, main.bbl)
