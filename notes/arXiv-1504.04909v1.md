...
- tags: [evolutionary-computation, quality-diversity, MAP-Elites, illumination-algorithm, soft-robotics]
...
- **hierarchical 版**（粗いセルから始めて時間とともに細かく分割）は実装上ほぼ必須レベルで、retina では 16×16 → 64×64 → 128×128 → 256×256 → 512×512 と段階的に細分化している（iteration 0, 1250, 2500, 5000）。

## Critical Thoughts（評価・疑問）

- **強み**:
  - アルゴリズムが本当に小さい（疑似コード十数行）。再実装の障壁が低く、これが MAP-Elites のその後の普及（QD 文献の事実上の base baseline 化）を支えている。
...
- **弱み / 疑問**:
  - 著者自身が冒頭の "Author's Note" で「preliminary draft、データは差し替わる予定」と明言している。実際、soft robot は runs 数が EA=7/EA+D=5/MAP-Elites=8 と不揃いで、real arm の評価回数は本文（640/729）と実験パラメータ節（420/512）で食い違う。Final 版が arXiv に上がっていないこと自体が再現性の課題。
  - **MOLE との定量比較が無い**（本文中で「MOLE 30 runs マージで初めて 1 run MAP-Elites 相当」と anecdotal にだけ書かれている）。最近接の先行手法と直接比較しないのは弱い。
  - **同じ評価予算での fair comparison が意識されていない**。MAP-Elites は archive 全体に評価を分散する一方、EA は数セルに集中する。global performance で勝つには「同じ予算なら集中型より分散型が良い」を示さねばならないが、本文は 1 run = N 評価という揃え方のみ。
...
- **次に試したいこと**:
  - 同じ評価予算で {MAP-Elites, NS+LC, MOLE, CMA-ES, EA+D} を比較する Pareto curve（横軸=評価数、縦軸=4 指標）。
  - feature dimensions を学習で取る（CVT-MAP-Elites や AURORA など後発の方向）と、本論文の手動 dimensions とで illumination の質がどう変わるか定量化。
  - hierarchical 版の split timing と final resolution の感度解析。retina の階段スケジュール（0/1250/2500/5000 で倍化）が他ドメインでも妥当なのか確認したい。
  - LLM 系のプロンプト/コード生成空間で feature = "コード長 × API 呼び出し回数" のようにして MAP-Elites を回す（最近の EvolutionThroughLargeModels 系の base としてどう機能するか）。
  - "anomalous island" のような simulator quirk を発見器として MAP-Elites を使い、シミュレータバグ検出ツールに転用。
