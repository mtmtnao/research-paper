...
- venue / year: TeX 中には明示なし（main.tex は neurips preprint style を使用）
...
- **手法**: Evolutionary Model Merge。merging を 2 つの直交する空間に分解して進化計算で同時最適化する。
  - **Parameter Space (PS)**: DARE-TIES の sparsification / weight mixing パラメータを置き、CMA-ES (Optuna 実装, σ=1/6, init=0.5, pop = 4 + ⌊3 ln n⌋) で最適化。報告された PS モデルは各 source model を単一 layer とみなし、各 model に 2 つの DARE-TIES 関連パラメータを割り当てる最も単純な設定。1000 trials。
...
- **結果（baseline 比較, Table 2）**: 未最適化 merging は TIES-Merge 4.4, DARE-TIES 35.2, Frankenmerging 0.0 (MGSM-JA)。LoRA fine-tune の最良は WizardMath ベースで 43.2（JP-LMEH は 55.9）。比較対象の中では Ours (PS+DFS) が MGSM-JA 最高、Ours (PS) が JP-LMEH 最高。
...
- **scaling matrix $W$ が DFS merging の要**：13B では $W$ なしの PS+DFS が 26.4 (model #6) で、$W$ ありの PS+DFS 34.0 (model #7) と PS 31.2 (model #3) を下回る。7B 解析でも $W_{ij}=1$ にすると 20% 以上の劣化。単に層を抜き挿しするだけだと層間で活性化分布がズレる、という著者の経験的主張と整合する。
...
- **弱み / 疑問**:
  ...
  - **MGSM-JA test set 250 問という小ささ**。信頼区間は TeX 中には示されていない。
  - **JA-VLM-Bench-In-the-Wild が 42 画像 50 問**。著者ら自身が作ったベンチで、サンプル数は小さい。GPT-4V で QA を作って人手フィルタしているため、評価データ作成過程への依存は残る（評者補足）。
...
- PS: 枠組みとしては TIES-Merging + DARE を layer-wise に張るが、報告された PS モデルは各 source model を単一 layer とみなし、DARE-TIES 関連 2 パラメータ × 3 source = 6 次元を CMA-ES で最適化（§3.1, §4.3 Analysis）。
...
- VLM: Japanese cultural content（鯉のぼり、鹿、信号機、原爆ドーム）でケーススタディ（Appendix C）。「青信号」を "青" と正しく答えられたのは EvoVLM-JP のみ、LLaVA は "green"、Japanese-Stable-VLM は不安定（緑/緑白/白）。
...
- (verified 2026-05-27) venue/year を TeX で確認できる範囲（main.tex の neurips preprint style 使用、掲載先・年の明示なし）に限定 (main.tex)
- (verified 2026-05-27) PS の説明を、報告モデルでは各 source model を単一 layer とみなす 6 次元設定であることに修正 (main.tex, Analysis)
- (verified 2026-05-27) baseline 比較の「唯一」および MGSM-JA の誤差範囲断定・商業的利害の推測を削除し、Table 2 / Discussion で確認できる記述に限定 (main.tex)
- (verified 2026-05-27) VLM ケーススタディの画像内容と Related Papers の年・タイトルを TeX / main.bbl で確認できる範囲に修正 (tables/vlm_samples.tex, main.bbl)
...
- Shi+ 2023 *Language models are multilingual chain-of-thought reasoners* / Cobbe+ 2021 *Training Verifiers to Solve Math Word Problems* — 数学評価。
...
- Sun+ 2024 *Transformer Layers as Painters* — DFS のモチベーション（隣接層 swap で性能低下）の予備調査。
