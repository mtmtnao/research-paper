...
- venue / year: arXiv preprint, 2019 (v3)
...
- **手法**: POET は環境–エージェントのペア集合 `EA_list` を維持し、各 iteration で (1) 既存環境を mutate して子環境を生成、(2) 各ペアを ES で 1 ステップ最適化、(3) 全ペア間で transfer 試行、を行う。新しい子環境は親エージェントの reward が再生産閾値（実験では 200）以上の親からのみ生まれ、Minimal Criterion `50 ≤ E^child(θ^child) ≤ 300`（簡単すぎず・難しすぎない）でフィルタした上で、ジェネティック L2 距離による novelty（k=5）でランキングして上位を受理。Transfer は `EVALUATE_CANDIDATES`: 各 candidate θ^m と「θ^m に E で 1 ES ステップ進めた proposal」を併せた 2M 個から、E 上の reward が最大のものを返し、現職を上回れば置換（"direct transfer" と "proposal transfer"）。最大同時環境数は cap、超過時は最古を queue 的に除去。Optimizer は ES（Salimans+ 2017）だが TRPO/PPO/GA でも置換可能と明記。
...
- 「single-sample t-test と Mann-Whitney U」を使い分けているのは open-ended 系特有の事情：POET は同じターゲットを再現的に生成しないので n=1 にしかならない。**open-ended 系の評価設計は通常の ML ベンチマークとは別物**として扱う必要がある。
...
- **弱み / 疑問**:
  ...
  - Direct-path curriculum control は「等しい計算予算」と言いつつ、何を予算に数えるかの定義（target の祖先環境すべての ES ステップ + transfer 計算を含む）が POET 側に有利寄りに見える。Curriculum 設計（毎ステップ確率 1/2 で parameter を mutation step だけ +）はかなり単純で、強い curriculum learning baseline（teacher-student, ALP-GMM 等）と比べた fair comparison は無い。
...
- **次に試したいこと**:
  - Encoding を CPPN / 文法 / wave function collapse など indirect にして、「max 値で頭打ち」現象が本当に解消するか確認。
  - Direct-path control をもっと真面目な curriculum learning（ALP-GMM, PLR, ACCEL）に差し替えて、POET の優位が残るか。実は ACCEL（Parker-Holder+ 2022）は POET のフォロワーで、ここに直接答えている系譜のはず。
  - Transfer の有効性を agent-side ではなく env-side の量で測る（祖先環境の集合がどれだけ系統的に複雑度を上げているか）。
  - POET 由来の transfer ログを policy distillation の教師にして、単体 RL agent が「環境ごとに歩行を切り替える generalist」に近づけるか。
  - Reward 関数自体をエンコードして共進化させる（discussion で言及されているが本論文では未実施）。
  - 「POET で生成された 25k iter ぶんの環境群」自体をベンチマークとして公開・固定化し、別アルゴリズムの解きやすさを比較するメタベンチに使えないか。
...
- Lehman & Stanley, *Novelty Search* (ECJ 2011) — genetic novelty による多様性圧力の元ネタ。
